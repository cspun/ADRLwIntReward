...
class SetVerbosity:
    ...
    def __enter__(self):
        ...
        self.gym_level = gym.logger.INFO 
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.verbose <= 0:
            logger.set_level(int(self.log_level))
            gym.logger.set_level(int(self.gym_level))
...
class TensorboardWriter:
    ...
    def __enter__(self):
        if self.tensorboard_log_path is not None:
            ...
            with self.graph.as_default(): 
                self.writer = tf.compat.v1.summary.FileWriter(save_path)
